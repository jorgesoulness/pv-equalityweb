...
class Singular {
	...
	public function maybe_save_expiration_date_on_frontend_submit( $post_id, $form_data ) {
		$settings = get_post_meta( $form_data['id'], FormExpirationSettingsModel::FORM_META_SETTING_NAME, true );
		...
		$form_slug = get_post_field( 'post_name', $form_data['id'] );//isset( $form->post_name ) ? $form->post_name : '';
		...
		$settings['action']['custom_actions'] = apply_filters( Action::CUSTOM_ACTIONS_FILTER_HANDLE . '_' . $form_data['id'], $settings['action']['custom_actions'], $post_id, $form_data );
		...
		$form_notifications = get_post_meta( $form_data['id'], Notifications::FORM_META, true );
