...
Toolset.CRED.shortcodeManager = function( $ ) {
	...
	self.initHooks = function() {
		...
		Toolset.hooks.addFilter( 'toolset-filter-shortcode-gui-cred_delete_post_link-computed-attribute-values', self.adjustDeletePostLinkAttributes, 10 );
	...
	self.initTemplates = function() {
		self.templates = {
			credIncluded: {
				...
				'cred_delete_post_link': wp.template( 'cred-delete-post-link-template' ),
	...
	self.associationFormShortcodeWizardAction = {
		error: function( responseData ) {
			...
			$( '.js-cred-shortcode-gui-button-afw-craft .ui-button-text' ).html( self.i18n.action.doContinue );
			$( '.js-cred-shortcode-gui-button-afw-craft' )
				.addClass( 'button-secondary' )
				.removeClass( 'button-primary' )
				.prop( 'disabled', true );
		...
		insertForm: function() {
			...
			switch( Toolset.hooks.applyFilters( 'toolset-filter-get-shortcode-gui-action', '' ) ) {
				case 'save':
					$( '.js-cred-shortcode-gui-button-afw-craft .ui-button-text' ).html( self.i18n.action.save );
					break;
				case 'create':
				case 'append':
					$( '.js-cred-shortcode-gui-button-afw-craft .ui-button-text' ).html( self.i18n.action.create );
					break;
				case 'edit':
					$( '.js-cred-shortcode-gui-button-afw-craft .ui-button-text' ).html( self.i18n.action.update );
					break;
				case 'insert':
				default:
					$( '.js-cred-shortcode-gui-button-afw-craft .ui-button-text' ).html( self.i18n.action.insert );
					break;
			}
	...
	self.associationFormShortcodeWizardOutcome = {
		...
		createCt: function ( dialogCompleteData ) {
			...
			$( '.js-cred-shortcode-gui-button-afw-craft .ui-button-text' ).html( self.i18n.action.processing );
			$( '.js-cred-shortcode-gui-button-afw-craft' )
				.addClass( 'button-secondary' )
				.removeClass( 'button-primary' )
				.prop( 'disabled', true );
	...
	/**
	 * Adjust the attributes for the delete post link shortcode.
	 *
	 * @since 1.9.3
	 */
	self.adjustDeletePostLinkAttributes = function( shortcodeAttributeValues, data ) {
		...
		if ( _.contains( [ 'reload', 'toolsetCombo' ], data.rawAttributes[ 'redirect' ] ) ) {
